...
export default function Page() {
  ...
  async function sendToAI(nextDecision = decision, nextTone = tone, nextHorizon = horizon) {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ decision: nextDecision, tone: nextTone, horizon: nextHorizon })
      });
  ...
  const downloadOrShareScreenshot = async () => {
    ...
    if (navigator.share && navigator.canShare?.({ files: [file] })) {
      await navigator.share({ files: [file], title: "Future Me Screenshot", text: caption });
      return;
    }
    ...
    <main style={styles.page}>
      <div style={styles.shell}>
        <header style={styles.topBar}>
          <button style={styles.iconButton} aria-label="Menu">≡</button>
          ...
          <div style={styles.topActions}>
            ...
            <button style={styles.iconButton} aria-label="More">⋯</button>
          </div>
        </header>
          ...
          <div style={styles.chatBody}>
            ...
            <div style={styles.messages}>
              {messages.map((msg, index) => (
                <div key={`${msg.from}-${index}`} style={{ ...styles.row, justifyContent: msg.from === "me" ? "flex-end" : "flex-start" }}>
                  <div style={{ ...styles.bubble, ...(msg.from === "me" ? styles.bubbleMe : styles.bubbleFuture) }}>
                    {msg.text}
          ...
          <div style={styles.composerTop}>
            <div style={styles.controlsRow}>
              <div style={styles.control}>
                ...
                <select value={template} onChange={(e) => { setTemplate(e.target.value); setDecision(e.target.value); }} style={styles.select}>
              ...
              <div style={styles.control}>
                ...
                <select value={tone} onChange={(e) => setTone(e.target.value as Tone)} style={styles.select}>
              ...
              <div style={styles.control}>
                ...
                <select value={horizon} onChange={(e) => setHorizon(e.target.value)} style={styles.select}>
            ...
            <div style={styles.templatePills}>
              {templates.slice(0, 4).map((t) => (
                <button key={t} style={styles.pill} onClick={() => { setTemplate(t); setDecision(t); }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          {composerOpen && (
            <div style={styles.composerBottom}>
              ...
              <div style={styles.footerHint}>
                Press Enter to send. The result is meant to feel like a real private chat, not an app demo.
              </div>
            </div>
          )}
        </div>
